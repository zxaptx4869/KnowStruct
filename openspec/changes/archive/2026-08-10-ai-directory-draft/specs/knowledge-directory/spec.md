...
系统 SHALL 在同一响应式 Web 应用中提供桌面和移动目录体验。桌面端 SHALL 使目录树与内容区
持续并置，并提供拖拽与等价菜单的排序 / 移动能力；移动端 SHALL 提供逐级浏览、创建子节点、
改名和节点操作菜单，MUST NOT 提供拖拽；节点操作菜单 SHALL 提供与桌面等价的编辑、移动到和
删除子树能力。
