...
|---|---|
| candidate | 跳过（已是候选） |
| open 且无处理记录 | 跳过（已在待处理列表） |
| open 且存在已解决/忽略记录 | 删除该处理记录，问题重新出现在待处理（保持 open，不重复确认） |
| rejected | 复活为候选（更新 scan/描述/严重度） |

- 处理记录查询按 `finding_type + target_type=ai_finding + target_id` 且 Workspace 隔离。
- 删除处理记录随扫描事务一起提交；不新增字段与迁移。
