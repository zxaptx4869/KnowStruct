...
图标入口，MUST 由用户手动触发，MUST NOT 自动执行。入口 MUST 仅在存在活跃草稿时隐藏，
其他情况下对任意存在节点的项目可见；目标节点为根节点或任意层级节点均可发起。
...
- **WHEN** 该项目存在起草中、待澄清、待确认或起草失败的草稿
- **THEN** 节点详情页不显示「AI 拓展建议」入口，草稿面板继续提供「继续处理 / 放弃草稿」
...
系统 SHALL 在发起拓展时创建目标节点归属的拓展草稿，并将项目背景、目标节点的现有子树
...
#### Scenario: Mark removals as not selected by default
- **WHEN** 差异面板展示建议移除项
- **THEN** 移除项默认不勾选，用户必须逐个显式勾选才会在确认时执行删除
...
系统 SHALL 为待确认拓展草稿提供与起草一致的会话式微调：信息不足时 SHALL 先进入澄清
（最多 5 个问题、选项为主、可跳过），之后每条消息 SHALL 追加会话历史，模型可只讨论
...
#### Scenario: Clarify before generating when context is thin
- **WHEN** 目标节点与项目背景信息不足
- **THEN** 界面展示最多 5 个澄清问题，可跳过直接生成
