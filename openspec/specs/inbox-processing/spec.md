...
系统 SHALL 在采集箱列表与详情中提供由 Processing Task 和 Extraction 派生的处理状态：处理中（待处理 / 执行中）、失败、待确认、已处理（全部候选已决定）。Source 自身的内容状态 MUST 与处理状态分离保存；列表 MUST 同时返回待确认、已接受、已拒绝候选数量，不得伪造未实现的数据。
...
系统 MUST 将 Processing Task 持久化在 MySQL 中，状态为待处理 / 处理中 / 处理成功 / 处理失败。任务失败时 MUST 保留原始 Source；重试 MUST 从失败步骤重新进入队列，且不得复制 Source 或已生成的候选 / Entry。
...
系统 SHALL 在桌面和 390px 移动视口提供同一套采集能力：全局"采集"入口支持文字与链接采集；项目内"添加资料"入口复用同一采集箱并预选该项目。提交中 MUST 阻止重复提交；明确失败 MUST 保留用户输入。

#### Scenario: Capture from the global entry
- **WHEN** 桌面或移动用户从全局导航进入采集箱并提交文字或链接
...
- **THEN** 界面保留已输入的内容、链接与项目选择，显示可执行的错误提示，并允许修正后重试
