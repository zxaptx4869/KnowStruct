...
系统 SHALL 通过可替换的 AI Provider 抽象为每个可提取 Source 生成一条或多条 Extraction 候选。Provider 实现 MUST 支持 OpenAI 兼容 SDK 与可配置 base_url（DeepSeek Provider），候选 SHALL 包含标题、内容、记录类型、适用条件、置信度与建议归档节点路径（可为空）。AI 输出 MUST 始终保存为待确认候选，不得直接写入正式 Entry。
...
#### Scenario: Choose the AI provider by configuration
- **WHEN** 系统按配置选用 DeepSeek Provider 处理提取
- **THEN** 请求使用配置的 API Key、base_url 与模型调用 OpenAI 兼容接口，结果经结构化校验后保存为候选

#### Scenario: Fail when the provider is not configured
- **WHEN** AI Provider 未配置 API Key 或配置无效
